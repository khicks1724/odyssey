-- Migration 058: normalize goal activity attachments and harden storage access
...
alter table public.goal_attachments
  add column if not exists comment_id uuid references public.goal_comments(id) on delete cascade;
alter table public.goal_attachments
...
create index if not exists idx_goal_attachments_goal_created
  on public.goal_attachments(goal_id, created_at desc);

create index if not exists idx_goal_attachments_comment_created
  on public.goal_attachments(comment_id, created_at asc)
