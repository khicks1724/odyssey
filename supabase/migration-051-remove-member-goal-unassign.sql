...
begin
  ...
  set
    ...
    assigned_to = case
      when g.assigned_to = old.user_id then cleaned.filtered_assignees[1]
      when g.assigned_to is not null and g.assigned_to = any(cleaned.filtered_assignees) then g.assigned_to
      else cleaned.filtered_assignees[1]
    end
  from (
    select
      goal.id,
      coalesce(
        array_remove(
          coalesce(
            goal.assignees,
            case
              when goal.assigned_to is not null then array[goal.assigned_to]
              else '{}'::text[]
            end
          ),
          old.user_id
    ...
    where goal.project_id = old.project_id
      and (
        ...
        or coalesce(goal.assignees, '{}'::text[]) @> array[old.user_id]
      )
  ) as cleaned
