-- migration-055: resolve project email invites against Odyssey account email as well as linked Microsoft integrations
...
begin
  ...
  if v_method = 'username' then
    ...
    from (
      ...
      from public.profiles p
      where lower(btrim(coalesce(p.email, ''))) = v_identifier

      union

      select au.id as user_id
      from auth.users au
      where lower(btrim(coalesce(au.email, ''))) = v_identifier
  ...
  if v_target_user_id is null then
    ...
    return jsonb_build_object('error', 'No Odyssey account was found for that Microsoft email.');
  end if;
